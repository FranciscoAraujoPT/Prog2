...
no_avl* avl_pesquisa_impl(no_avl *no, const int v);
no_avl* avl_insere_impl(no_avl *no, const int v);
no_avl* avl_remove_impl(no_avl *no, const int v);
no_avl* avl_no_valormin(no_avl* no);
no_avl* avl_no_valormax(no_avl* no);
void avl_preordem_impl(no_avl *no);
void avl_inordem_impl(no_avl *no);
...
void avl_apaga(arvore_avl* avl)
{
    ...
    while(avl->raiz != NULL)
    {
        avl->raiz = avl_remove_impl(avl->raiz, avl->raiz->valor);
    }
...
no_avl* avl_pesquisa(arvore_avl* avl, const int v)
{
    ...
    return avl_pesquisa_impl(avl->raiz, v);
}
...
int avl_insere(arvore_avl* avl, const int v)
{
    ...
    avl->raiz = avl_insere_impl(avl->raiz, v);
...
int avl_remove(arvore_avl* avl, const int v)
{
    ...
    avl->raiz = avl_remove_impl(avl->raiz, v);
...
int avl_min(arvore_avl* avl)
{
    ...
    return avl_no_valormin(avl->raiz)->valor;
}
...
int avl_max(arvore_avl* avl)
{
    ...
    return avl_no_valormax(avl->raiz)->valor;
}
...
no_avl* avl_novo_no(const int v)
{
...
no_avl* avl_pesquisa_impl(no_avl* no, const int v)
{
    ...
    if(v < no->valor)
        return avl_pesquisa_impl(no->esquerda, v);

    else if(v > no->valor)
        return avl_pesquisa_impl(no->direita, v);
...
no_avl* avl_insere_impl(no_avl *no, const int v)
{
    ...
    if (no == NULL)
        return avl_novo_no(v);

    if (v < no->valor)
        no->esquerda  = avl_insere_impl(no->esquerda, v);
    else if(v > no->valor)
        no->direita = avl_insere_impl(no->direita, v);
    else {
        return no;
    }
    ...
    else
    {
        ...
        else
        {
           ...
           no->valor = temp->valor;
            ...
            no->direita = avl_remove_impl(no->direita, temp->valor);
        }
    }
