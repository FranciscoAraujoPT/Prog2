...
int tabela_insere(tabela_dispersao *td, const char *chave, int valor)
{
    int index;
    td_elemento * elem;
...
int tabela_existe(tabela_dispersao *td, const char *chave)
{
    ...
    objeto *o = tabela_valor(td, chave);

    if (!o) return TABDISPERSAO_NAOEXISTE;

    return TABDISPERSAO_EXISTE;
}

objeto* tabela_valor(tabela_dispersao *td, const char *chave)
{
    int index;
    td_elemento * elem;
